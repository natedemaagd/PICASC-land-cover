...
# define landcovers
landcovers <- c(5,6,8,10,12,13,14,15,16,17,31,32,33,34,35,39,41,42)
...
for(i in 1:length(landcovers)){
    ...
    # save output
    saveRDS(reg_spatial_final, file = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Water yield/02_regressions/02_regressions_landcover', landcovers[[i]], '.rds'))
    gc()
...
ggsave(filename = 'H:/My Drive/Projects/PICASC Land-to-sea/Figures and tables/Figures/Water yield/02_regressions_residualsByLandcoverType_gls.png')
...
reg_data_lm <- list()
for(i in 1:length(landcovers)){
  reg_data_lm[[i]] <- lm(formula = reg_eqn, data = dat[dat$LC == landcovers[[i]],])
}

# get residuals from each LC type
reg_resids_lm <- lapply(reg_data_lm, function(reg) data.frame(resid = reg$residuals))
for(i in 1:length(reg_resids_lm)){ reg_resids_lm[[i]]$LC = landcovers[[i]]}
