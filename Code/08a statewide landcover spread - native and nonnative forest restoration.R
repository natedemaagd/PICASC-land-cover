...
# spreads native and non-native forest
...
simulation_label <- "_07%spread_100 years_250m"
#Parameters
#infested_value
infest_val <- 2000
#susceptible_values 
suscep <- as.integer(c(1:2, 30, 33:35)) + 1000
#spread_rate 
spredrate <- growrate <- 0.075
#bird_rate 
birdcell <- 0.00
...
reg_results <- readRDS("H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Water yield/02_regressions/02_regressions_landcover8_PtSm.rds")
...
##### spread in protected areas only #####

# this section alters the LC raster values:
  # forest that spreads will have value of 2000
  # LC that is susceptible to spread has value of 1000 + original LC value
  # LC that is not susceptible has original LC value

# since we are spreading multiple types of native forest at once, need to convert some of the native forest values (see `infest_val` above)
LC[LC %in% c(8:15, 31:32, 36)] <- 1000  # only add 1000 because an extra 1000 is added below for pixels in protected areas


# we want to let the reforestation run only within protected areas. Add 1000 to these values to separate them from the rest
shp_protectedAreas_lonlat <- st_zm(shp_protectedAreas_lonlat)   # coerce shapefile to two-dimensional polygons
shp_protectedAreas_lonlat <- shp_protectedAreas_lonlat[!st_is_empty(shp_protectedAreas_lonlat),,drop=FALSE]  # remove empty polygons
LC_protected <- mask(x = LC, mask = shp_protectedAreas_lonlat)  # find pixels within protected areas
LC_protected[!is.na(LC_protected)] <- 1000  # if a pixel in in a protected area, give it a value of 1000
LC_protected[is.na(LC_protected)] <- 0  # if pixel is not in protected area, give it a value of 0
LC <- LC + LC_protected  # change LC value in protected area to original LC value + 1000
...
save.image("H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Water yield/07 statewide landcover spread/sim_1_to_1000_nativeAndNonNativeForestRestoration.RData")
...
for (i in 1:simlength) {
  ...
  writeRaster(last_raster, paste0("H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Water yield/07 statewide landcover spread/rasters/Native and non-native forest restoration - 7.5 pct spread rate - landcover rasters/finalRRaster",
