library(rgdal); library(raster); library(gstat); library(ggplot2)
...
moku <- readOGR('D:/OneDrive - hawaii.edu/Documents/Projects/Data/Shapefiles/Moku-shp/Moku-ridge-to-reef', 'moku_ridge_to_reef_dar')
moku <- spTransform(moku, sp::CRS('+proj=longlat +zone=4 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs'))
...
dat_sp <- SpatialPointsDataFrame(coords = dat[c('POINT_X', 'POINT_Y')], proj4string = crs(moku), data = dat)
