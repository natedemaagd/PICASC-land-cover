...
# list rasters
list_rasters <- list.files('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Water yield/08 statewide landcover spread - forest restoration/rasters/Native forest restoration - 7.5 pct spread rate - landcover rasters',
                           pattern = '.tif', full.names = TRUE)

# re-order rasters according to year
vec_names <- sapply(strsplit(list_rasters, ' '), function(l) l[[23]])
...
list_dfs <- lapply(list_dfs,   # basic 'native', 'non-native', 'other' categorization
                   function(df){
                     df$`Land cover` <- ifelse(df$lc %in% c(1:2, 30:36) &
                                                 !is.na(df$lc),
                                               'Grass and shrubland',
                                        ifelse(df$lc %in% c(1000) &
                                                 !is.na(df$lc),
                                               'Forest',
                                               'Other')
                                        )
...
foreach(i = 1:length(list_dfs),
  ...
  # save plot
  ggsave(plot = p,
         filename = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Water yield/08 statewide landcover spread - reforestation/landcover spread timelapse individual yearly plots - native forest/',
...
# list PNGs
list_pngs <- list.files('H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Water yield/08 statewide landcover spread - reforestation/landcover spread timelapse individual yearly plots - native forest',
                        pattern = '.png',
                        full.names = TRUE)
list_pngs %>%
  ...
  image_write("H:/My Drive/Projects/PICASC Land-to-sea/Figures and tables/Figures/Water yield/08 native forest reforestation timelapse 7pct.gif")
